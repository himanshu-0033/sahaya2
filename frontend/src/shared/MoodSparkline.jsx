...
export default function MoodSparkline({ history, height = 40, barWidth = 8 }) {
  ...
  return (
    <div className="flex items-end gap-[2px]" style={{ height }} role="img" aria-label="Mood trend, most recent check-ins">
      {history.map((entry) => {
        const barHeight = Math.max(4, (entry.moodScore / 5) * height);
        return (
          <div
            key={entry.date}
            title={`${entry.date} — ${moodLabel(entry.moodScore)}`}
